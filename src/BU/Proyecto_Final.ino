...
void loop() {
  ...
  {
    delay(1000); // Delay para tomar las mediciones de 2 segundos
    ...
    for (int cont_current =0; cont_current<=100; cont_current++)
      {
    ...
    float corriente_final = promedio_I/100;
    potencia = (corriente_final/1000)*5;  

    Serial.print("Corriente de motor:   "); Serial.print(current_mA); Serial.println(" mA");
    Serial.println("");
    ...
    if(t<=22)
    {
      ...
      if (cont_tiempo_normal ==  5)  //En caso de que el contador llegue al limite
      {
    ...
    if(t>22)
    {
      ...
      if (cont_tiempo_medio ==  5) //En caso de que el contador llegue al limite
      {
    ...
    if (cont_tiempo_nube == 5) //Contador para publicarlo en Google Sheets
    {
...
//Funcion de calculo de corriente